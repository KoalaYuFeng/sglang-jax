...
from sgl_jax.srt.layers.deepseek_v4.compressor import compress, physical_locations
...
def attention(
    ...
    *,
    ...
    q = linear(qr, weights, "attn.wq_b").reshape(
        x.shape[0], config.heads, config.head_dim
    )
    if dense_kernels.fused_norm:
        ...
        kv = jnp.concatenate(
            (
                activation_fp8_roundtrip(kv[:, : -config.rope_dim], 64),
                kv[:, -config.rope_dim :],
            ),
            axis=-1,
        )
    ...
    logical_window = (
        jnp.maximum(positions[:, None] - config.window + 1, 0)
        + jnp.arange(config.window)[None]
    )
    window = physical_locations(metadata, metadata.token_requests, logical_window)
    window = jnp.where(
        (logical_window <= positions[:, None]) & metadata.token_valid[:, None],
        window,
        -1,
    )
    indices = window
    ...
    if config.ratio:
        ...
        if config.ratio == 4:
            ...
            def select(item):
                ...
                physical = (
                    physical_locations(
                        metadata, request[None], terminal_positions[None]
                    )[0]
                    // config.ratio
                )
                ...
                score = round_bf16(
                    jnp.einsum(
                        "hd,td->ht", query, keys, preferred_element_type=jnp.float32
                    )
                )
                score = round_bf16(
                    jnp.maximum(score.astype(jnp.float32), 0)
                    * mixing.astype(jnp.float32)[:, None]
                )
                score = round_bf16(
                    jnp.sum(score.astype(jnp.float32)[None], axis=1)[0]
                ).astype(jnp.float32)
                score = jnp.where(logical_candidates < length, score, -jnp.inf)
                return (
                    score,
                    jax.lax.top_k(score, min(config.index_topk, candidate_count))[1],
                )
            ...
            if trace is not None:
                trace.update(
                    index_q=iq, index_score=index_score, index_selected=selected
                )
        else:
            selected = jnp.broadcast_to(
                logical_candidates, (x.shape[0], candidate_count)
            )
        physical = (
            physical_locations(
                metadata, metadata.token_requests, (selected + 1) * config.ratio - 1
            )
            // config.ratio
        )
        compressed_valid = (
        ...
        if not (
            ...
            all_kv = jnp.concatenate(
                (cache["window"], cache["main.compressed"]), axis=0
            )
    ...
    if dense_kernels.fused_wo_a:
        projected = projection_kernels.inverse_rope_fp8_wo_a(
            value,
            weight,
            scales,
            cosine,
        ...
        value = rope(value, positions, config, inverse=True).reshape(
            x.shape[0], config.groups, -1
        )
        projected = [
            low_bit_matmul(
                value[:, group],
                ...
                scales[
                    group * config.o_rank // 128 : (group + 1) * config.o_rank // 128
                ],
                weight_format="fp8",
    ...
    if output_tensor_axis is not None:
        projected = jax.lax.all_gather(
            projected, output_tensor_axis, axis=1, tiled=True
        )
