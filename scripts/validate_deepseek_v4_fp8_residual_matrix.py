"""Shape and four-device controls for residual-preserving diagnostic adapters."""
...
def main():
    ...
    parser.add_argument("--start", type=int, choices=(0, 64), required=True)
    ...
    report = {
        ...
        "positions": [args.start, min(args.start + 63, 126)],
        "cases": [],
    }
    ...
    valid = min(64, 127 - args.start)
    x = np.pad(native["qr"][args.start : args.start + valid], ((0, 64 - valid), (0, 0)))
    ...
    try:
        ...
        np.testing.assert_array_equal(
            baseline_bf16[:valid],
            native["attn.wq_b.output"][args.start : args.start + valid],
        )
        ...
        variants = [
            (f"residual{k}", ResidualDiagnostic(split_k=k, preserve_bf16_residual=True))
            for k in (16, 8)
        ]
        for name, adapter in variants:
            ...
            for rows, tile_m in ((64, 32), (8, 8)):
                ...
                entry = {
                    ...
                    "carrier_row_subset_matches_full": bool(
